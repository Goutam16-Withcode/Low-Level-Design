...
public:
    ...
    { // this is convention using the name with
        req.url = u;
        return *this;
    }
    ...
    // Build method to create the immutable HttpRequest object --> terminating method
    HttpRequest build()
    {
        if (req.url.empty())
        ...
        if (req.timeout == -1)
            throw runtime_error("Timeout must be specified");
